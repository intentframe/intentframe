...
| [gray_area_dry_run.txt](./gray_area_dry_run.txt) | Dry-run result for the 20-intent gray-area dev workflow suite under `test_policy_root_admin_assistant.yaml`. All fixtures expect `ALLOW`; the run captures which sensitive-but-useful workflows Guardian permits or blocks in root context. |
| [gray_area_dry_run_IF_logs.txt](./gray_area_dry_run_IF_logs.txt) | Core runtime log for the gray-area dry-run, including Command Shield, Analysis Engine, and Guardian rationale for each intent. |
| [gray-area-case-study.md](./gray-area-case-study.md) | Minimal interpretation of the gray-area result: what passed, what blocked, and why the suite matters for policy judgment around normal developer work with sensitive capabilities. |
