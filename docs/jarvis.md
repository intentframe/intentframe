...
| Can I disable a tool? | Yes — at the policy layer, not in Jarvis. The runtime policy defaults are seeded at gateway startup from [`../intentframe_gateway/bootstrap.py`](../intentframe_gateway/bootstrap.py); [`../jarvis_pa/seed_policies.py`](../jarvis_pa/seed_policies.py) is a dev mirror of the same shape. Today, customising means editing one of those source files and re-running bootstrap; a file/CLI-based policy editor is on the roadmap. The tool stays callable from Jarvis's side; every invocation is denied at the boundary. |
