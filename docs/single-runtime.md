...
The singleton boundary is the **security lane**: one runtime process tree protects one machine/environment. The `intentframe-server` pipeline serializes the full lifecycle of each intent — evaluation, decision, execution, and audit — so each action is judged against a stable environment and recorded in causal order. Executor `worker_pool.max_workers` is only a capacity ceiling inside the executor service; it is not the security boundary and must not be used to justify multiple writers into the same environment.
