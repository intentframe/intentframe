...
- Every `RUN_COMMAND` subprocess is wrapped in a **kernel-enforced macOS Seatbelt SBPL profile** (`intentframe_native_kit/intentframe_executor_pack_macos/sandbox/`) launched via `sandbox-exec`. The profile is dynamically built per execution from the planner's capability set, with `(deny default)` and a curated allowlist. Even root-UID subprocesses cannot violate the profile without a kernel exploit.
