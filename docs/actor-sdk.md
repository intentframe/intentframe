...
| Can I bypass the Actor SDK for performance? | You can — but then IntentFrame doesn't see the action and offers no protection for it. The fast path for safe reads (`READ_FILE`, `LIST_DIRECTORY`, etc.) already resolves in milliseconds without any AI call; the cost of going through the SDK is small. See [faq.md § Q5](faq.md#q5-what-is-the-latency-cost). |
