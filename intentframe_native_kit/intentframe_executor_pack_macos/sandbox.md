...
Implementation lives in `intentframe_native_kit.intentframe_executor_pack_macos/sandbox/`. Core `executor/` passes an opaque `sandbox:` dict from YAML; `TerminalAdapter` owns engine/planner setup.
...
```
intentframe_native_kit.intentframe_executor_pack_macos/sandbox/
...
Wiring: `intentframe_native_kit.intentframe_executor_pack_macos/adapters/terminal.py` creates the engine and planner when sandbox is enabled.
...
`intentframe_native_kit.intentframe_executor_pack_macos/sandbox/pathing.py` provides `canonical_sandbox_path()` which the planner applies to:
...
The profile is generated entirely in Python code (`intentframe_native_kit.intentframe_executor_pack_macos/sandbox/__init__.py`), following Anthropic's `sandbox-runtime` pattern. There is no static `.sbpl` file.

---

## Classifier (Library Only)

The classifier (`intentframe_native_kit.intentframe_executor_pack_macos/sandbox/classifier.py`) performs deterministic, shlex-based analysis of command strings. It is **not used in the execution path** — the planner applies the same template to all commands regardless of what the classifier would say.
