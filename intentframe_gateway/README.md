...
| 6 | Start supervisor | Passes the first-party kit profile (`intentframe_native_kit/supervisor_profile.yaml`, override with `INTENTFRAME_SUPERVISOR_CONFIG`), sets `INTENTFRAME_CORE_CONFIG` and `EXECUTOR_CONFIG` on the child env (see `profiles.py` and [docs/plugin-profiles.md](../docs/plugin-profiles.md)), spawns 4 infra services (policy-registry, resource-registry, executor, intentframe-core); inherits `INTENTFRAME_ESCALATION_ARMED` |
| 7 | Start platform server (macOS) | Via `open .app` for TCC permissions; non-fatal if unavailable |
| 8 | Bootstrap | Seed policies and workspace (idempotent); policy seed validation uses the same `core.yaml` bundle list as core will load |
...
├── profiles.py            # resolve_core_config_path() — shared gateway core profile path
