...
| 6 | Start supervisor | Passes the first-party kit profile (kit `supervisor_profile.yaml` in the installed package, override with `INTENTFRAME_SUPERVISOR_CONFIG`), sets `INTENTFRAME_CORE_CONFIG` and `EXECUTOR_CONFIG` on the child env (see `profiles.py` and [docs/plugin-profiles.md](../docs/plugin-profiles.md)), spawns 4 infra services (policy-registry, resource-registry, executor, intentframe-server); inherits `INTENTFRAME_ESCALATION_ARMED` |
