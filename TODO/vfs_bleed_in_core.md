...
The deletion domain schema is still path-shaped, so non-file destructive actions don't fit the abstraction. Schemas now live in **`intentframe_native_kit/action_registry/domains/`** (not `intentframe_core`) — core only keeps the `DomainSchema` base:

```9:15:intentframe_native_kit/action_registry/domains/deletion.py
Current limitation:
...
And `intentframe_native_kit.action_registry/types.py` has `DELETE_EVENT`/`DELETE_REMINDER` **commented out** of `ACTION_DOMAINS` precisely because the path-shaped deletion schema can't represent them. The file vocabulary still constrains the taxonomy even though the schema no longer bleeds into core.
...
The cleaner end-state the code itself gestures at: move identity/path canonicalization to a neutral `intentframe_core.identity`, treat VFS as a files-bundle-owned capability the executor resolves, and generalize the deletion domain off `path`. Domain intent schemas have already moved from `intentframe_core` to `intentframe_native_kit.action_registry/domains/` (core keeps only `DomainSchema` base). Until VFS and path canonicalization follow, **VFS is a file-family concern that the substrate carries on its behalf** — which is precisely the bleed you're sensing.