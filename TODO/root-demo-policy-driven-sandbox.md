...
✅ Full 100-intent sweeps after remediation — `demo/tests/root_demo/results/dry_run.txt` records 100/100 expected `BLOCK` with `INTENTFRAME_EXECUTOR_MODE=dry_run INTENTFRAME_DRY_RUN_CONTEXT=root`; `demo/tests/root_demo/results/real_run.txt` records 100/100 expected `BLOCK` against the real root-capable executor path with preflight `whoami == root`.  This is now the publishable proof point for the root-demo attack corpus.
