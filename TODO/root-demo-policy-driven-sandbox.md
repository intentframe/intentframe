...
Phase 7a shipped (narrowed scope) — see `TODO/jarvis-write-file-policy-and-python-env.md` for the full record.  Floor parity for WRITE_FILE / DELETE_FILE (the actual gap — VFS was bypassing the sandbox's `NON_NEGOTIABLE_DENY_WRITE`) via `resource_registry/floor.py` + `LocalVirtualFileSystem` enforcement is in.  `FileIntel` is computed once in the pipeline and forwarded to AE as trusted context (rendered as the `WRITE_FILE — PAYLOAD SIGNALS` block) and to Guardian's deterministic checkers; the Guardian LLM prompt is intentionally unchanged.  `DeterministicGuardian` hard-BLOCKs WRITE_FILE to sensitive virtual paths (`matched_gate="write_file_sensitive_path"`) and otherwise falls through to AE + AIGuardian.  The `critical_write_file` AE lane exists and every WRITE_FILE routes onto it, but its overlay body is still aliased to `_STANDARD` — net effect on the LLM surface is byte-identical to pre-Phase-7a outside the new trusted-context block.  **Explicitly NOT shipped:** the earlier passive-write DG `ALLOW` fast-path (reverted: content-based ALLOWs over agent-controlled inputs were judged too fragile) and the payload-aware lane split (`critical_write_file_code` vs `critical_write_file`; deferred until per-lane overlay bodies are authored — tracked as xfailed tests).  Original "payload inspection + destination sensitivity policy" framing retracted: path policy is the semantic authorization, the floor is the non-negotiable symmetric-with-sandbox guarantee, DG gates sensitive virtual paths, and AE-side reasoning over `FileIntel` is defense-in-depth on top — not the primary gate.
