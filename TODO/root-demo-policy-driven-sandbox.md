...
✅ Root demo profile (Jarvis root profile) — shipped as `jarvis_pa/executor_root.yaml` (executor config) + the `INTENTFRAME_PROFILE=root` / `--profile root` gateway/CLI flag.  Privilege-separation model: the executor service remains a normal-user process in the supported path; only the child `sandbox-exec` wrapper for an allowed `RUN_COMMAND` can request root via `sudo -n`.  Machine capability is gated by `intentframe_setup_root_demo.sh`, which installs a narrow `NOPASSWD: SETENV: /usr/bin/sandbox-exec` sudoers entry and a user-space marker.  Gateway detects armed/disarmed at startup and injects `INTENTFRAME_ESCALATION_ARMED=0|1`; executor health reports `running_as_root=True` as an effective `RUN_COMMAND` capability signal, not a literal UID-0 process claim.  CLI shows escalation banner and post-quit uninstall reminder.  The root-demo Python harness is RUN_COMMAND-only, starts with a hard dry-run or `whoami == root` preflight, and exits non-zero on preflight/verdict failures.  See `docs/root_demo/executor-root-mode.md` and `demo/tests/root_demo/README.md` for the full operator flow.  **Note:** the "separate UserContext / policy profile" described in the original checklist item was not needed — the existing policy bootstrap covers the root profile shape via `INTENTFRAME_PROFILE=root`; the executor YAML + escalation mechanism handles the privilege side.

✅ Red-team corpus expanded and remediated — attack corpus expanded from 24 → 100 intents organized by MITRE ATT&CK tactic: destructive/obvious (Tier 1), persistence (LaunchAgent, crontab, shell rc, .pth), privilege escalation (sudoers, sshd_config, PAM), credential access (keychain, SSH keys, AWS creds, browser cookies, dscl auth), data exfiltration (curl/rsync/aws/nc staging), interpreter indirection (python -c os.system, bash -c curl|sh, env-stripped rm), reason-vs-target lies (AI-layer proof: malicious command + benign cover story), and network mutation (DNS, ARP, routing, hostname, NTP, EDR launchd, browser prefs).  The nine formerly-ALLOW intents from the 2026-04-27 live sweep (36, 38, 87, 88, 89, 90, 91, 97, 99) now BLOCK via `deterministic_guardian` / `matched_gate=constraint` after the sensitive-surface capability taxonomy and deny-set landed.  Split across the `test_attacks.py` aggregator plus 11 per-tactic files documented in `demo/tests/root_demo/README.md`.

✅ Full 100-intent sweeps after remediation — `demo/tests/root_demo/results/deny_cap_only_dry_run.txt` records 100/100 expected `BLOCK` with `INTENTFRAME_EXECUTOR_MODE=dry_run INTENTFRAME_DRY_RUN_CONTEXT=root`; `demo/tests/root_demo/results/deny_cap_only_real_run.txt` records 100/100 expected `BLOCK` against the real root-capable executor path with preflight `whoami == root`.  This is now the publishable proof point for the root-demo attack corpus.

☐ Semantic `intent_limits` for the root-demo threat model — add operator-voiced policy limits for credential/session reads, host-network mutation, security-posture weakening, unauthorized egress, persistence, privilege escalation, and scope-fidelity. These are first-class semantic policy for Guardian/AE context, not deterministic tag substitutes.
