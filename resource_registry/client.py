...
class ResourceRegistryClient:
    ...
    def __init__(
        ...
        *,
        ...
        ``base_url`` (or the ``INTENTFRAME_RESOURCE_URL`` env var) routes
        through the IntentFrame edge to a remote resource-registry.  When
        unset, the client falls back to the local UDS at ``socket_path``.
        """
        import os
        base_url = base_url or os.environ.get("INTENTFRAME_RESOURCE_URL")
        if base_url:
            self._socket = None
